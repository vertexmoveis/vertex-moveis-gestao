...
test('commercial list deduplicates requests, preserves alternatives and scopes every view', async () => {
  ...
  try {
    ...
    assert.deepEqual(all.counts, { all: 5, active: 3, closed: 1, lost: 0, sold: 1 })
    ...
    assert.equal((await list({ view: 'closed' })).total, 1)
    ...
    await db.exec(`INSERT INTO "QuoteGroup" VALUES ('lost-business','Pedido perdido','c','a',NULL,'2026-09-14');
      INSERT INTO "Quote" VALUES ('only-lost','lost-business','LOST',400,NULL,'2026-09-14',NULL,1,6,'Padrão',NULL)`)
    const lost = await list({ view: 'lost' })
    assert.equal(lost.total, 1)
    assert.equal(lost.items[0].quoteId, 'only-lost')
    assert.equal(lost.counts.lost, 1)
    const closed = await list({ view: 'closed' })
    assert.equal(closed.total, 1)
    assert.deepEqual(new Set(closed.items.map(item => item.status)), new Set(['CANCELLED']))
    const sold = await list({ view: 'sold' })
    assert.equal(sold.total, 1)
    assert.equal(sold.items[0].quoteId, 'legacy-q')
    assert.equal(sold.counts.sold, 1)
    assert.equal((await list({ view: 'sold', userId: 'b' })).total, 0)
    assert.equal((await list({ view: 'lost', userId: 'b' })).total, 0)
    assert.equal((await list({ view: 'lost', query: 'Cozinha' })).total, 0)
    assert.equal((await list({ view: 'lost', archived: true })).items[0].quoteId, 'archived-q')
